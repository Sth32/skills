...
| `game-client-handoff` | `<主题名>-客户端对接文档.md` | 从 Server 事实生成属性同步、实体生命周期、RPC/事件和数据充分性合同，不设计客户端内部实现 |
...
`game-client-handoff` 是独立的对接阶段，因此拥有自己的唯一文档。它必须放在本次主题已有 `01` 至 `06` 阶段文档所在目录，并命名为 `<主题名>-客户端对接文档.md`；可在属性和 RPC 骨架稳定后先生成，并在完整实现后原位更新，不得再拆分属性清单、RPC 清单或客户端缺口报告。
