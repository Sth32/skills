...
**分支 Review 输入规则：过程工作流若存在 `00-工作流索引.md`，必须用它定位全部实际工作单元，并确认本次纳入长期文档的叶工作单元都已完成各自 `07-Uxx-*` Review。整体 `07-交叉评审.md` 仅在确有集成风险时存在；存在则必须达到可交付终态并作为额外证据，不存在不得单独成为阻塞。`00` 只用于定位材料，不得把 Uxx、DAG、阶段状态或任务进度写入长期权威文档。**
